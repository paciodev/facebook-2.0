    ...
    <div className="bg-white p-2 rounded-2xl shadow-md text-gray-500 font-medium mt-6">
      ...
      <div className="flex justify-evenly p-3 border-t">
        <div className="inputIcon">
          ...
          <p className="hidden sm:inline text-sm xl:text-base">Live Video</p>
        </div>
        <div onClick={() => filePickerRef.current.click()} className="inputIcon cursor-pointer">
          <CameraIcon className="h-7 text-green-400" />
          <p className="hidden sm:inline text-sm xl:text-base">Photo/Video</p>
          <input hidden type="file" onChange={addImageToPost} ref={filePickerRef} />
        </div>
        <div className="inputIcon">
          <EmojiHappyIcon className="h-7 text-yellow-300" />
          <p className="hidden sm:inline text-sm xl:text-base">Feeling/Activity</p>
        </div>
      </div>
    </div>
  )
